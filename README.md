...
| `PS17062` | `PS17062`, 20-character UID, `uuid_type=1` | Real ARM64 hardware confirmed the native TMT IOTC/RDT 3.1.5.33 route, automatic `READ STATUS`, and `FULL OPEN` with ACK plus live opening/final-open telemetry | ✅ **Native route active:** automatic status polling and the APK-derived 23-parameter UART0 profile are enabled over the same persistent IOTC/RDT session. Parameter changes use fresh full read → one full-frame write (no retry) → mandatory full readback equality. Normal cover movement remains disabled apart from the explicit one-shot hardware-test action until the remaining movement commands are confirmed |
