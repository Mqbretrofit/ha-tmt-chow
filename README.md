...
| `PS17062` | `PS17062`, 20-character UID, `uuid_type=1` | Real ARM64 hardware confirmed the native TMT IOTC/RDT 3.1.5.33 route and `READ STATUS` reply (`ACK STATUS:PED CLOSED,2`); `RS` is rejected | ✅ **Status route verified:** automatic native `READ STATUS` polling is implemented. Normal cover movement remains disabled; a separate explicitly confirmed `tmt_chow.ps17062_movement_test` action is available for one-shot hardware validation. Parameter writes and normal runtime parameter polling remain disabled |
